...
| 作りたいもの | 手段 | 参照 |
|---|---|---|
| 写真風・イラストの**背景ビジュアル**（バナー、LPのヒーロー、SNS投稿の絵） | `image_gen`（AI生成） | セクション0〜6 |
| **ロゴ・見出し・コピー** | 実ファイル＋実フォントで**後から合成**（HTML＋Chrome） | セクション7 |
| **インフォグラフィック・図解・比較表・手順図** | **HTML＋Chrome のみ。AI生成は使わない** | **セクション10** |
| **アイコン** | AI生成→ベクター化（質が高い）／既製セット（速い）。**AIのラスターをそのまま貼らない** | **セクション10** |
| データのグラフ・チャート | HTML/SVG＋`dataviz` スキル＋**実データ**（数字は作らない） | セクション10 |
| 実際の地図・地形 | 地理データSVG（パブリックドメイン等）を調達。アイコンセットには無い | — |
| 人物の顔を似せる | `image_gen` は参照画像を取れない。ビジョン言語化リレー | セクション9 |
| **UI・画面モック**（雰囲気の確認だけ） | `image_gen`（例外扱い）。**画面構成の詰めと実装への受け渡しは Claude Design が本筋** | **セクション11** |
...
### Figma で編集させたい場合: PNG＋SVG の2形式で出す
...
| **レビュー**（人／エージェント判断が必要） | 交通ルールなどの現地整合、媒体適合、ブランド適合、AI破綻、審査リスク |

### 呼び出し方

**配信面が決まっている場合は、生成前に1回**（仕様を出させる）＋**合成後に1回**（実物をレビューさせる）。

| 状況 | 呼ぶエージェント |
|---|---|
| ビジュアル仕様・媒体別の作り分け | `mkt-visual-creative` |
| Paid配信の審査・入稿要件が絡む | `mkt-paid` |
| オーガニック投稿の設計 | `mkt-social` |
| 複数媒体・複数区分にまたがる | `mkt-cmo` に振り分けさせる |

レビュー依頼時に**必ず渡すもの**: 合成後の画像パス／配信面（媒体・Paid or オーガニック）／出力サイズ／
...
8. ロゴ・文言が必要なら合成する（セクション7）。Figmaで編集させるならPNG＋SVGの2形式で出す
9. **配信面が決まっているなら、マーケレビューを通す（セクション7.5）**。
   指摘は「合成やり直し」か「1点変更で再生成」に振り分ける
